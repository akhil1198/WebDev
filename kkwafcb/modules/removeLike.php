<?php
    ...
    if(isset($_SESSION['user_id']) && isset($_SESSION['user_type'])){
        if(isset($_GET['blog_id'])){
            ...
            $query = 'SELECT * FROM liked_posts WHERE user_id = '.$user_id.' AND blog_id = '.$blog_id;
            $execute = mysqli_query($CONN,$query);
            echo mysqli_num_rows($execute);
            if(mysqli_num_rows($execute) == 0){
                header('LOCATION:http://koniandko/modules/likedPosts.php');
            }
            $query = 'DELETE FROM liked_posts WHERE user_id = '.$user_id.' AND blog_id = '.$blog_id;
            $execute = mysqli_query($CONN,$query);
            if(!$execute){
